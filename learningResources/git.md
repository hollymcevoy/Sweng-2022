...
Instead, you will create ***speciality branches*** on your machine that exist for the purpose of solving singular issues. You will always base your new branches of the development branch. Speciality branches is a general term for branches used for some specific purpose. Typically they are feature branches. They can also be branches for bug fixes and hot fixes. It is sometimes usefulto include some signature in the name of a branch to let others know what sort of branch it is. 

		git checkout develop
		git checkout -b my-feature-branch

This last command creates a new branch named "my-feature-branch" based off of develop. You can name that branch whatever you like. You should not have to push it to Github unless you intend to work on multiple machines on that feature. The feature branch can be forked further as the features developed become more complex. You may end up creating a sub-feature branch, with many branches branching off from that too. The point here is that the amount of branches you make is your own decision,
...
Working with a team
------------
If you have followed the above steps you may be comfortable working on solo projects with git and github. 
However working with a team requires slightly more care. As multiple people have access to the code and the code may be changing over time. In large projects the code might be changing every 30 minutes. So keeping track of the most recent truth of the program is important. 

There are two main points to remember when you are working on a team
- You should update your local copy before starting any work
- You should update your local copy before pushing any work to the remote repository. 

#### Before you start
If your local copy is not a brand new clone then you should update before you start working. 
- Check the status of the remote repository
- Update if neccesary
- Begin to work
There may be other situations in this workflow where things do not go like this. Merge conflicts can occur. The linked documentation above should contain solutions to resolving them.  


#### Before you push	
If you have completed some work and wish to push
- Check the status of the remote repository
- Update if neccesary
- Merge your changes
- Push to remote
